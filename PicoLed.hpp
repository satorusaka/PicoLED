...
template <typename T>
static PicoLedController addLeds(PIO pioBlock, uint stateMachine, uint dataPin, uint numLeds, DataByte b1, DataByte b2, DataByte b3, DataByte b4, bool isInverted) {
    uint pioIndex;
    if (pioBlock == pio0) {
        pioIndex = 0;
    ...
    shared_ptr<T> target(new T(pioBlock, stateMachine, dataPin, numLeds, b1, b2, b3, b4, isInverted));
    targets[pioIndex][stateMachine] = target;
    return PicoLedController(target);
};

template <typename T>
static PicoLedController addLeds(PIO pioBlock, uint stateMachine, uint dataPin, uint numLeds, DataFormat format, bool isInverted) {
    switch (format) {
        case FORMAT_RGB:
            return addLeds<T>(pioBlock, stateMachine, dataPin, numLeds, RED, GREEN, BLUE, NONE, isInverted);
        default:
        case FORMAT_GRB:
            return addLeds<T>(pioBlock, stateMachine, dataPin, numLeds, GREEN, RED, BLUE, NONE, isInverted);
        case FORMAT_WRGB:
            return addLeds<T>(pioBlock, stateMachine, dataPin, numLeds, RED, GREEN, BLUE, WHITE, isInverted);
    }
}
